...
if __name__ == '__main__':
    ...
    try:
        df = pandas.read_sql(
            sql='SELECT repo_full_name FROM tag_recommender_ground_truth_repo_topics ORDER BY repo_full_name', con=conn)
        for i, r in df.iterrows():
            ...
            try:
                if (readme_json_object['name'] == 'README.md'):
                    ...
                    logging.info(
                        'Repository {0} uses README other than README.md ({1}). Skipping'.format(repo_full_name,
                                                                                                 readme_json_object[
                                                                                                     'name']))
            except KeyError:
